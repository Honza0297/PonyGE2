...
def mapper(genome, tree, params):
    ...
    if genome:
        ...
        if params['GENOME_OPERATIONS']:
            ...
            used_codons = map_ind_from_genome(genome, params=params)
            ...
            if params["ATTRIBUTE_GRAMMAR"]:
                phenotype, genome, tree, nodes, invalid, depth, \
                    used_codons = map_codetree_from_genome(genome, params=params)
            else:
                phenotype, genome, tree, nodes, invalid, depth, \
                    used_codons = map_tree_from_genome(genome, params=params)
...
def map_ind_from_genome(genome, params):
    """
...
def map_tree_from_genome(genome, params=None):
    """
    ...
    tree = Tree(str(params['BNF_GRAMMAR'].start_rule["symbol"]), None, params=params)

    # Map tree from the given genome
    output, used_codons, nodes, depth, max_depth, invalid = \
        genome_tree_map(tree, genome, [], 0, 0, 0, 0, params=params)
...
def map_codetree_from_genome(genome, params):
    """
    Maps a full tree from a given genome.
    ...
    # Initialise an instance of the tree class
    tree = CodeTree(params=params, root=str(params['BNF_GRAMMAR'].start_rule["symbol"]))

    # Map tree from the given genome
    output, used_codons, nodes, depth, max_depth, invalid = \
        genome_codetree_map(tree, genome, [], 0, 0, 0, 0, params=params)
...
def genome_codetree_map(tree, genome, output, index, depth, max_depth, nodes,
    ...
    if not invalid and index < len(genome) * (params['MAX_WRAPS'] + 1):
        ...
        tree.build_node(nt_name=tree.root, processed_code=chosen_prod["attr_code"], rhs_sequence=children)
        # was tree.build_node(nt_name=tree.root, raw_code=chosen_prod["attr_code"][1:-1], rhs_sequence=children)
        ...
        for symbol, rhs_symbol in zip(chosen_prod['choice'], tree.rhs):
            ...
            if symbol["symbol"] == "<postconditions>":
                pass
            if symbol["type"] == "T":
                # Append the child to the parent node. Child is a terminal, do
                # not recurse.
                tree.children.append(CodeTree(root=symbol["symbol"], lhs=rhs_symbol, parent=tree, params=params))
...
def genome_tree_map(tree, genome, output, index, depth, max_depth, nodes,
    ...
    if not invalid and index < len(genome) * (params['MAX_WRAPS'] + 1):
        ...
        if params["ATTRIBUTE_GRAMMAR"]:
            ...
            tree.code = chosen_prod["attr_code"]
        ...
        for symbol in chosen_prod['choice']:
            ...
            if symbol["type"] == "T":
                # Append the child to the parent node. Child is a terminal, do
                # not recurse.
                tree.children.append(Tree(symbol["symbol"], tree, params=params))
                output.append(symbol["symbol"])
