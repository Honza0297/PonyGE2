...
def initialisation(size, agent):
    ...
    # the limit.
    size -= len(agent.GE_params['SEED_INDIVIDUALS'])

    # Initialise empty population.
    individuals = agent.GE_params['INITIALISATION'](size, agent)

    # Add seed individuals (if any) to current population.
    individuals.extend(agent.GE_params['SEED_INDIVIDUALS'])

    return individuals


def sample_genome(agent):
    """
    Generate a random genome, uniformly.
    
    :return: A randomly generated genome.
    """
    genome = [randint(0, agent.GE_params['CODON_SIZE']) for _ in
              range(agent.GE_params['INIT_GENOME_LENGTH'])]
    return genome


def uniform_genome(size, agent):
    """
    ...
    return [individual.Individual(genome=sample_genome(agent), ind_tree=None, agent=agent) for _ in range(size)]
...
def seed_individuals(size):
    ...
    no_seeds = len(agent.GE_params['SEED_INDIVIDUALS'])
    ...
    if no_seeds > 0:
        ...
        for ind in agent.GE_params['SEED_INDIVIDUALS']:
...
def rhh(size):
    ...
    if size < 2:
        ...
        # If we have no depths to ramp from, then agent.GE_params['MAX_INIT_DEPTH'] is
...
def generate_PI_ind_tree(max_depth):
    ...
    # Initialise an instance of the tree class
    # !  Totok je important
    ind_tree = Tree(str(agent.GE_params['BNF_GRAMMAR'].start_rule["symbol"]), None)
        ...
        if genotype:
            ...
            if phenotype and ind.phenotype != phenotype:
                s = "scripts.seed_PonyGE2.load_population\n" \
                    ...
                    agent.GE_params['GRAMMAR_FILE']
                raise Exception(s)

        else:
            # Set target for GE LR Parser.
            agent.GE_params['REVERSE_MAPPING_TARGET'] = phenotype
...
def LTGE_initialisation(size):
    ...
    for _ in range(size):
        # Random genotype
        g, ph = latent_tree_random_ind(agent.GE_params['BNF_GRAMMAR'],
                                       agent.GE_params['MAX_TREE_DEPTH'])
